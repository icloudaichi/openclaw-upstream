...
- Typing indicators in groups follow `agents.defaults.typingMode`. Automatic visible replies post normal final answers after mention and allowlist gates pass. If you configure `messages.groupChat.visibleReplies: "message_tool"`, typing starts immediately by default so group members can see the agent is working even if no automatic final reply is posted. Explicit typing-mode config still wins.
